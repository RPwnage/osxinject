...
mach_error_t mach_inject(const mach_inject_entry	threadEntry, const void *paramBlock, size_t paramSize, pid_t targetProcess, vm_size_t stackSize){
	...
	if( !err ) {
		...
		if (err == 5) fprintf(stderr, "[ERROR] Could not access task for pid %d. You probably need to add user to procmod group\n", targetProcess);
#endif
	}
	...
	if(!err && paramBlock != NULL && paramSize){
		...
		if(!err){
			...
			err = vm_write(remoteTask, remoteParamBlock, (pointer_t) paramBlock, (unsigned int)paramSize);
			printf("\nWrote param with size %d\n", (int)paramSize);
		}
	}
...
mach_error_t machImageForPointer(const void *pointer, const void **image, unsigned long *size, unsigned int *jumpTableOffset, unsigned int *jumpTableSize){
	assert( pointer );
	assert( image );
	assert( size );
	...
	for( imageIndex = 0; imageIndex < imageCount; imageIndex++ ) {
#if defined(__x86_64__)
		const struct mach_header_64 *header = (const struct mach_header_64 *)_dyld_get_image_header((uint32_t)imageIndex); // why no function that returns mach_header_64
		...
		long start = section->addr + _dyld_get_image_vmaddr_slide( (uint32_t)imageIndex);
		long stop = start + section->size;
		if( p >= start && p <= stop ) {
			const char *imageName = _dyld_get_image_name((uint32_t)imageIndex);
            __unused const struct mach_header *imageHeader = _dyld_get_image_header((uint32_t)imageIndex);
			printf("\tBootstrap:\t%s\n", imageName);
			assert(imageName);
			struct stat sb;
...
#if defined(__i386__)
void* fixedUpImageFromImage (const void *image, unsigned long imageSize, unsigned int jumpTableOffset, unsigned int jumpTableSize, ptrdiff_t fixUpOffset){
